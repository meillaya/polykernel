...
- **Not done here, by design**: running it. The verification block needs a CUDA
  ...
  (PolyKernel builds CUDA kernels but has no local NVIDIA GPU, and Triton is not
  in the nix dev shell). Running the tutorial on a GPU box is the first step at
  submission time.
