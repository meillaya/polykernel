//===- cpu_ref_silu.cpp - CPU reference SiLU driver -------------*- C++ -*-===//
//
// PolyKernel CPU reference SiLU driver (Todo 9 / Wave 2).
//
//===----------------------------------------------------------------------===//
//
// File-based validation bridge (Pinned contract B): reads a bf16 input .npy,
// computes SiLU (x / (1 + exp(-x))) via launch_silu_cpu (the same math + rounding
...
//
// Usage:
//   cpu_ref_silu INPUT.npy OUTPUT.npy
//
// SiLU is elementwise + shape-preserving, so the whole tensor is flattened to a
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    polykernel::cpu::launch_silu_cpu(input.data.data(), output.data(),
                                     input.size());
